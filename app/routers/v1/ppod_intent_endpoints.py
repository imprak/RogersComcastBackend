...
async def create(
    ...
    db_obj = cruds.ppod_intent_cruds.create(db=db, data_in=data_in)
...
def get_multi(
    ...
    db_objs = cruds.ppod_intent_cruds.get_multi(db=db, page=page, page_size=page_size)
    return JSONResponse(
...
def delete(
    partner_id: str = Path(alias="partnerId"),
    client_id: str = Query(None, alias="clientId"),
    ppod_intent_id: uuid.UUID = Query(alias="ppodIntentId"),
    db: Session = Depends(get_db),
):
    ...
    if not db_obj:
        err = f"Ppod intent {ppod_intent_id} not found"
        log.error(err)
        return JSONResponse(
